...
| `beads_verify <plan.json>` | Print the WBS-sorted **blocks** table for the plan's beads (scoped via the manifest): each bead's blocked-by set, a `GATE` column (`run`/`manual`/`—`), a single-letter `M` tier column (`w`/`m`/`l`), a `V` review column (`R` = flagged `review:req`, `✓` = `review:ok`, `✗` = `review:block`), a `TIME SPENT` column, a `WHY` column (per-bead justification from the plan's `why`, else the tier default), `← READY` flags, and cross-plan anomalies — followed by an embedded legend. **`TIME SPENT` is posterior: blank until a bead is closed, then filled from `bd history` as the claim→close (`in_progress`→`closed`) span. This is _elapsed wall-clock, not active effort_ — a bead left claimed overnight counts that idle time; `~` marks a create→close fallback when no claim was recorded.** Open beads trigger no history lookup, so the column costs nothing mid-run. Auto-invoked by `beads_gen`; also runnable standalone. `bd ready` stays authoritative. |
...
| `render_mermaid.py <manifest>` | Render the review chart (helper) — edge colours + gate badges (🔒/👁) + review badge (🔎 = `review:req`) + model-tier tags (`[w]`/`[m]`; none = lead). |
